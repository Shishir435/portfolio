...
const AuthAdminCheck = ({setIsAdmin}:{setIsAdmin:Dispatch<SetStateAction<boolean>>}) => {
    const [username, setUsername] = useState("")
    const [password, setPassword] = useState("")
    const [usernameType, setUsernameType] = useState<"text"|"password">("password")
  ...
  async function handleAdminValidation(username:string,password:string){
    try {
        ...
        if (admin.username === username && admin.password === password) {
          ...
          localStorage.setItem('adminData', JSON.stringify({ username, password, expiration }));
        } else {
          setIsAdmin(false);
          alert("This site is reserved for administrators only. Please refrain from accessing this page if you are not an admin.");
        }
      } catch (error:any) {
  ...
  return (
    ...
    <h1 className="text-xl sm:text-2xl md:text-3xl lg:text-4xl text-center">"This site is exclusively for administrators."</h1>
    <Dialog>
        ...
        <div className="grid gap-4 py-4">
          ...
          <div className="grid grid-cols-4 items-center gap-4">
            ...
            <Input
              ...
              required
            />
            <div className='absolute right-8' onClick={()=>{
                setUsernameType(usernameType==="password"?"text":"password")
            }}>
